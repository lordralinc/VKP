...
import dev.idm.vkp.model.AccessIdPair;
...
import dev.idm.vkp.model.criteria.PhotoAlbumsCriteria;
...
public class PhotosInteractor implements IPhotosInteractor {
    ...
    public Single<List<Photo>> getPhotosByIds(int accountId, Collection<AccessIdPair> ids) {
        List<dev.idm.vkp.api.model.AccessIdPair> dtoPairs = new ArrayList<>(ids.size());

        for (AccessIdPair pair : ids) {
            dtoPairs.add(new dev.idm.vkp.api.model.AccessIdPair(pair.getId(),
