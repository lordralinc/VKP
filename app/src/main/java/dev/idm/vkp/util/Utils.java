...
import dev.idm.vkp.api.ProxyUtil;
import dev.idm.vkp.api.model.Identificable;
import dev.idm.vkp.media.exo.OkHttpDataSourceFactory;
import dev.idm.vkp.model.ISelectable;
import dev.idm.vkp.model.ISomeones;
...
public class Utils {
    ...
    public static boolean isHiddenCurrent() {
        return isHiddenType(dev.idm.vkp.settings.Settings.get().accounts().getType(dev.idm.vkp.settings.Settings.get().accounts().getCurrent()));
    }

    public static boolean isHiddenAccount(int account_id) {
        return isHiddenType(dev.idm.vkp.settings.Settings.get().accounts().getType(account_id));
    }
    ...
    public static int getThemeColor(boolean isOfReadToast) {
        switch (dev.idm.vkp.settings.Settings.get().ui().getMainThemeKey()) {
            case "fire":
            case "yellow_violet":
            case "fire_gray":
    ...
    public static Context updateActivityContext(Context base) {
        ...
        int size = dev.idm.vkp.settings.Settings.get().main().getFontSize();
        @Lang int lang = dev.idm.vkp.settings.Settings.get().other().getLanguage();
        if (size == 0) {
    ...
    public static void checkMusicInPC(Context context) {
        if (!AppPerms.hasReadWriteStoragePermission(context))
            return;
        File audios = new File(dev.idm.vkp.settings.Settings.get().other().getMusicDir(), "local_server_audio_list.json");
        if (!audios.exists())
            return;
        try {
